...
let sendResourceGroupRequest = ev => {
  ...
  async function addResourceGroupToHTML() {
    ...
    const locationOptions = ["eastus", "East US", "eastus2", "East US 2", "centralus", "Central US", "northcentralus", "North Central US", "southcentralus", "South Central US", "westus", "West US", "westus2", "West US 2", "westus3", "West US 3"];
...
function createResourceGroup(RGName, RGlocation) {
  ...
  async function createRG() {
    let created = false;
    fetch(createRGRequest).then(response => response.json()).then(data => {
      console.log(data);
      created = true;
    }).catch(error => {
      console.log("Error message:");
...
let sendServerRequest = ev => {
  ...
  async function fetchServer() {
    fetch(ServerRequest).then(response => response.json()).then(data => {
      console.log(data);
      ServerInfo = data;
    }).catch(error => {
      console.log("Error message:");
...
let sendDBResourceGroupRequest = ev => {
  ...
  async function fetchDBResourceGroup() {
    fetch(DBResourceGroupRequest).then(response => response.json()).then(data => {
      console.log(data);
